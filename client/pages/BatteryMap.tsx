...
export default function BatteryMap() {
  ...
  const [selectedStation, setSelectedStation] = useState<BatteryStation | null>(
    null,
  );
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setUserLocation({ lat: 28.5881, lng: 77.347 });
        },
      );
    }
  }, []);
  ...
  const getStatusBadge = (status: string, availableBatteries: number) => {
    if (status === "maintenance") {
      ...
      return (
        <Badge className="bg-orange-100 text-orange-800">No Batteries</Badge>
      );
  ...
  const filteredStations = batteryStations.filter((station) => {
    ...
    if (filterStatus === "available")
      return station.status === "online" && station.availableBatteries > 0;
    return true;
  });
                  ...
                  <div className="absolute inset-0 bg-gradient-to-br from-green-100 to-blue-100 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-gray-600 mb-4">
                        Real-time battery station locations
                      </p>
                      ...
                      <span>
                        Available (
                        {
                          filteredStations.filter(
                            (s) =>
                              s.status === "online" && s.availableBatteries > 0,
                          ).length
                        }
                        )
                      </span>
                      ...
                      <span>
                        Limited (
                        {
                          filteredStations.filter(
                            (s) =>
                              s.status === "online" &&
                              s.availableBatteries <= 2 &&
                              s.availableBatteries > 0,
                          ).length
                        }
                        )
                      </span>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="text-lg font-bold">
                        {selectedStation.estimatedTime} min
                      </div>
